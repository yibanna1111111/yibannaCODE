'''
元宝阅读活动入口微信打开：http://mr1693645959906.kgtpecv.cn/coin/index.html?mid=RG5RC7LDU

http://u.cocozx.cn/api/coin/info
抓包活动链接 http://u.cocozx.cn/api/coin/info接口的请求体中的un和token参数
...
if __name__ == '__main__':
    ...
    try:
        ...
        exit(0)
    ybconfig = os.getenv('ybconfig')
    if ybconfig == None:
        print('请检查你的星空脚本变量名称是否填写正确')
        exit(0)
    try:
        ybconfig = json.loads(ybconfig.replace("'", '"'))
    except Exception as e:
        print(e)
        print(ybconfig)
        print('请检查你的星空脚本变量参数是否填写正确')
        exit(0)
